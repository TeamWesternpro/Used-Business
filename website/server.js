...
function loadConfig() {
    try {
        ...
        return {
            ...
            listingsWebhook: parsed.listingsWebhook || '',
            bookingsWebhook: parsed.bookingsWebhook || ''
        };
    } catch (e) {
        return { businessTypes: ['Dealership', 'Mechanic', 'Shops', 'Houses'], tagOptions: ['Available', 'Not Available'], listingsWebhook: '', bookingsWebhook: '' };
    }
}
...
const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === '/api/webhooks') {
        ...
        if (req.method === 'POST') {
            ...
            if (body && body.listings !== undefined) current.listingsWebhook = body.listings || '';
            if (body && body.bookings !== undefined) current.bookingsWebhook = body.bookings || '';
            saveConfig(current);
            sendJSON(res, 200, { success: true, webhooks: { listings: current.listingsWebhook, bookings: current.bookingsWebhook } });
            return;
        }
