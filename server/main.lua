...
local function SyncFromWebsite()
    if syncInProgress then
        if Config.Debug then
            print('[UsedDealership] Sync already in progress, skipping duplicate request')
        end
        return
    end
    if not Config.WebsiteURL or Config.WebsiteURL == '' then
        if Config.Debug then
            print('[UsedDealership] Website sync skipped: No URL configured in config.lua')
        end
        return
    end
    syncInProgress = true
    ...
    if Config.Debug then
        print('[UsedDealership] Syncing from website: ' .. url)
    end
    PerformHttpRequest(url, function(statusCode, text)
        syncInProgress = false
        if Config.Debug then
            print('[UsedDealership] Website sync response: HTTP ' .. tostring(statusCode))
        end
        if statusCode == 200 and text then
            ...
            if ok and data and data.success and data.listings then
                ...
                if Config.Debug then
                    print('[UsedDealership] Website sync failed: Invalid response data')
                end
            end
        else
            if Config.Debug then
                print('[UsedDealership] Website sync failed: HTTP ' .. tostring(statusCode))
            end
        end
...
local function PushToWebsite(method, data)
    if not Config.WebsiteURL or Config.WebsiteURL == '' then
        if Config.Debug then
            print('[UsedDealership] Push to website skipped: No URL configured in config.lua')
        end
        return
    end
    local url = Config.WebsiteURL .. '/api/listings'
    ...
    if Config.Debug then
        print('[UsedDealership] Pushing to website [' .. method .. ']: ' .. url)
    end
    PerformHttpRequest(url, function(statusCode, text)
        if Config.Debug then
            print('[UsedDealership] Website push response: HTTP ' .. tostring(statusCode))
        end
...
AddEventHandler('onResourceStart', function(resourceName)
    if GetCurrentResourceName() == resourceName then
        ...
        if Config.WebsiteURL and Config.WebsiteURL ~= '' then
